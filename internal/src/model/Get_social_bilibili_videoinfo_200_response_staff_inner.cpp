/**
 * UAPI
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 1.0.0
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.17.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */



#include "CppRestOpenAPIClient/model/Get_social_bilibili_videoinfo_200_response_staff_inner.h"

namespace org {
namespace openapitools {
namespace client {
namespace model {

Get_social_bilibili_videoinfo_200_response_staff_inner::Get_social_bilibili_videoinfo_200_response_staff_inner()
{
    m_Mid = 0.0;
    m_MidIsSet = false;
    m_Title = utility::conversions::to_string_t("");
    m_TitleIsSet = false;
    m_Name = utility::conversions::to_string_t("");
    m_NameIsSet = false;
    m_Face = utility::conversions::to_string_t("");
    m_FaceIsSet = false;
    m_Follower = 0.0;
    m_FollowerIsSet = false;
}

Get_social_bilibili_videoinfo_200_response_staff_inner::~Get_social_bilibili_videoinfo_200_response_staff_inner()
{
}

void Get_social_bilibili_videoinfo_200_response_staff_inner::validate()
{
    // TODO: implement validation
}

web::json::value Get_social_bilibili_videoinfo_200_response_staff_inner::toJson() const
{
    web::json::value val = web::json::value::object();
    if(m_MidIsSet)
    {   
        
        val[utility::conversions::to_string_t(_XPLATSTR("mid"))] = ModelBase::toJson(m_Mid);
    }
    if(m_TitleIsSet)
    {   
        
        val[utility::conversions::to_string_t(_XPLATSTR("title"))] = ModelBase::toJson(m_Title);
    }
    if(m_NameIsSet)
    {   
        
        val[utility::conversions::to_string_t(_XPLATSTR("name"))] = ModelBase::toJson(m_Name);
    }
    if(m_FaceIsSet)
    {   
        
        val[utility::conversions::to_string_t(_XPLATSTR("face"))] = ModelBase::toJson(m_Face);
    }
    if(m_FollowerIsSet)
    {   
        
        val[utility::conversions::to_string_t(_XPLATSTR("follower"))] = ModelBase::toJson(m_Follower);
    }

    return val;
}

bool Get_social_bilibili_videoinfo_200_response_staff_inner::fromJson(const web::json::value& val)
{
    bool ok = true;
    if(val.has_field(utility::conversions::to_string_t(_XPLATSTR("mid"))))
    {
        const web::json::value& fieldValue = val.at(utility::conversions::to_string_t(_XPLATSTR("mid")));
        if(!fieldValue.is_null())
        {
            double refVal_setMid;
            ok &= ModelBase::fromJson(fieldValue, refVal_setMid);
            setMid(refVal_setMid);
            
        }
    }
    if(val.has_field(utility::conversions::to_string_t(_XPLATSTR("title"))))
    {
        const web::json::value& fieldValue = val.at(utility::conversions::to_string_t(_XPLATSTR("title")));
        if(!fieldValue.is_null())
        {
            utility::string_t refVal_setTitle;
            ok &= ModelBase::fromJson(fieldValue, refVal_setTitle);
            setTitle(refVal_setTitle);
            
        }
    }
    if(val.has_field(utility::conversions::to_string_t(_XPLATSTR("name"))))
    {
        const web::json::value& fieldValue = val.at(utility::conversions::to_string_t(_XPLATSTR("name")));
        if(!fieldValue.is_null())
        {
            utility::string_t refVal_setName;
            ok &= ModelBase::fromJson(fieldValue, refVal_setName);
            setName(refVal_setName);
            
        }
    }
    if(val.has_field(utility::conversions::to_string_t(_XPLATSTR("face"))))
    {
        const web::json::value& fieldValue = val.at(utility::conversions::to_string_t(_XPLATSTR("face")));
        if(!fieldValue.is_null())
        {
            utility::string_t refVal_setFace;
            ok &= ModelBase::fromJson(fieldValue, refVal_setFace);
            setFace(refVal_setFace);
            
        }
    }
    if(val.has_field(utility::conversions::to_string_t(_XPLATSTR("follower"))))
    {
        const web::json::value& fieldValue = val.at(utility::conversions::to_string_t(_XPLATSTR("follower")));
        if(!fieldValue.is_null())
        {
            double refVal_setFollower;
            ok &= ModelBase::fromJson(fieldValue, refVal_setFollower);
            setFollower(refVal_setFollower);
            
        }
    }
    return ok;
}

void Get_social_bilibili_videoinfo_200_response_staff_inner::toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix) const
{
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t(_XPLATSTR(".")))
    {
        namePrefix += utility::conversions::to_string_t(_XPLATSTR("."));
    }
    if(m_MidIsSet)
    {
        multipart->add(ModelBase::toHttpContent(namePrefix + utility::conversions::to_string_t(_XPLATSTR("mid")), m_Mid));
    }
    if(m_TitleIsSet)
    {
        multipart->add(ModelBase::toHttpContent(namePrefix + utility::conversions::to_string_t(_XPLATSTR("title")), m_Title));
    }
    if(m_NameIsSet)
    {
        multipart->add(ModelBase::toHttpContent(namePrefix + utility::conversions::to_string_t(_XPLATSTR("name")), m_Name));
    }
    if(m_FaceIsSet)
    {
        multipart->add(ModelBase::toHttpContent(namePrefix + utility::conversions::to_string_t(_XPLATSTR("face")), m_Face));
    }
    if(m_FollowerIsSet)
    {
        multipart->add(ModelBase::toHttpContent(namePrefix + utility::conversions::to_string_t(_XPLATSTR("follower")), m_Follower));
    }
}

bool Get_social_bilibili_videoinfo_200_response_staff_inner::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    bool ok = true;
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t(_XPLATSTR(".")))
    {
        namePrefix += utility::conversions::to_string_t(_XPLATSTR("."));
    }

    if(multipart->hasContent(utility::conversions::to_string_t(_XPLATSTR("mid"))))
    {
        double refVal_setMid;
        ok &= ModelBase::fromHttpContent(multipart->getContent(utility::conversions::to_string_t(_XPLATSTR("mid"))), refVal_setMid );
        setMid(refVal_setMid);
    }
    if(multipart->hasContent(utility::conversions::to_string_t(_XPLATSTR("title"))))
    {
        utility::string_t refVal_setTitle;
        ok &= ModelBase::fromHttpContent(multipart->getContent(utility::conversions::to_string_t(_XPLATSTR("title"))), refVal_setTitle );
        setTitle(refVal_setTitle);
    }
    if(multipart->hasContent(utility::conversions::to_string_t(_XPLATSTR("name"))))
    {
        utility::string_t refVal_setName;
        ok &= ModelBase::fromHttpContent(multipart->getContent(utility::conversions::to_string_t(_XPLATSTR("name"))), refVal_setName );
        setName(refVal_setName);
    }
    if(multipart->hasContent(utility::conversions::to_string_t(_XPLATSTR("face"))))
    {
        utility::string_t refVal_setFace;
        ok &= ModelBase::fromHttpContent(multipart->getContent(utility::conversions::to_string_t(_XPLATSTR("face"))), refVal_setFace );
        setFace(refVal_setFace);
    }
    if(multipart->hasContent(utility::conversions::to_string_t(_XPLATSTR("follower"))))
    {
        double refVal_setFollower;
        ok &= ModelBase::fromHttpContent(multipart->getContent(utility::conversions::to_string_t(_XPLATSTR("follower"))), refVal_setFollower );
        setFollower(refVal_setFollower);
    }
    return ok;
}


double Get_social_bilibili_videoinfo_200_response_staff_inner::getMid() const
{
    return m_Mid;
}

void Get_social_bilibili_videoinfo_200_response_staff_inner::setMid(double value)
{
    m_Mid = value;
    m_MidIsSet = true;
}

bool Get_social_bilibili_videoinfo_200_response_staff_inner::midIsSet() const
{
    return m_MidIsSet;
}

void Get_social_bilibili_videoinfo_200_response_staff_inner::unsetMid()
{
    m_MidIsSet = false;
}
utility::string_t Get_social_bilibili_videoinfo_200_response_staff_inner::getTitle() const
{
    return m_Title;
}


void Get_social_bilibili_videoinfo_200_response_staff_inner::setTitle(const utility::string_t& value)
{
    m_Title = value;
    m_TitleIsSet = true;
}

bool Get_social_bilibili_videoinfo_200_response_staff_inner::titleIsSet() const
{
    return m_TitleIsSet;
}

void Get_social_bilibili_videoinfo_200_response_staff_inner::unsetTitle()
{
    m_TitleIsSet = false;
}
utility::string_t Get_social_bilibili_videoinfo_200_response_staff_inner::getName() const
{
    return m_Name;
}


void Get_social_bilibili_videoinfo_200_response_staff_inner::setName(const utility::string_t& value)
{
    m_Name = value;
    m_NameIsSet = true;
}

bool Get_social_bilibili_videoinfo_200_response_staff_inner::nameIsSet() const
{
    return m_NameIsSet;
}

void Get_social_bilibili_videoinfo_200_response_staff_inner::unsetName()
{
    m_NameIsSet = false;
}
utility::string_t Get_social_bilibili_videoinfo_200_response_staff_inner::getFace() const
{
    return m_Face;
}


void Get_social_bilibili_videoinfo_200_response_staff_inner::setFace(const utility::string_t& value)
{
    m_Face = value;
    m_FaceIsSet = true;
}

bool Get_social_bilibili_videoinfo_200_response_staff_inner::faceIsSet() const
{
    return m_FaceIsSet;
}

void Get_social_bilibili_videoinfo_200_response_staff_inner::unsetFace()
{
    m_FaceIsSet = false;
}
double Get_social_bilibili_videoinfo_200_response_staff_inner::getFollower() const
{
    return m_Follower;
}

void Get_social_bilibili_videoinfo_200_response_staff_inner::setFollower(double value)
{
    m_Follower = value;
    m_FollowerIsSet = true;
}

bool Get_social_bilibili_videoinfo_200_response_staff_inner::followerIsSet() const
{
    return m_FollowerIsSet;
}

void Get_social_bilibili_videoinfo_200_response_staff_inner::unsetFollower()
{
    m_FollowerIsSet = false;
}

}
}
}
}


